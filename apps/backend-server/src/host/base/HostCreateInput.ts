/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { AccommodationCreateNestedManyWithoutHostsInput } from "./AccommodationCreateNestedManyWithoutHostsInput";
import { Type } from "class-transformer";
import { ActivityCreateNestedManyWithoutHostsInput } from "./ActivityCreateNestedManyWithoutHostsInput";

@InputType()
class HostCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  password?: string | null;

  @ApiProperty({
    required: false,
    type: () => AccommodationCreateNestedManyWithoutHostsInput,
  })
  @ValidateNested()
  @Type(() => AccommodationCreateNestedManyWithoutHostsInput)
  @IsOptional()
  @Field(() => AccommodationCreateNestedManyWithoutHostsInput, {
    nullable: true,
  })
  accommodations?: AccommodationCreateNestedManyWithoutHostsInput;

  @ApiProperty({
    required: false,
    type: () => ActivityCreateNestedManyWithoutHostsInput,
  })
  @ValidateNested()
  @Type(() => ActivityCreateNestedManyWithoutHostsInput)
  @IsOptional()
  @Field(() => ActivityCreateNestedManyWithoutHostsInput, {
    nullable: true,
  })
  activities?: ActivityCreateNestedManyWithoutHostsInput;
}

export { HostCreateInput as HostCreateInput };
